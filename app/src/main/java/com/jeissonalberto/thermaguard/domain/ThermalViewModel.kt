...
class ThermalViewModel(application: Application) : AndroidViewModel(application) {
        ...
        viewModelScope.launch {
            ...
            while (true) {
                try {
                    ...
                    val prediction = SiliconPhysicsEngine.predictFuture(snapshot, SiliconPhysicsEngine.detectDevicePhysicsParams(), _uiState.value.history)
    ...
    fun applyPhysicsGovernor() = viewModelScope.launch {
        try {
            ...
            val govConfig = com.jeissonalberto.thermaguard.data.GovernorConfig(
            ...
            val results = CpuGpuGovernor.applyGovernorConfig(govConfig)
    ...
    fun emergencyThrottleIfNeeded() = viewModelScope.launch {
        try {
            ...
            if (temp >= 52f) {
                val results = CpuGpuGovernor.emergencyThrottle()
                _uiState.update { it.copy(governorLog = results) }
    ...
    fun refreshPhysicsAnalysis() = viewModelScope.launch {
        try {
            val snap = CpuGpuGovernor.buildFullThermalSnapshot(
